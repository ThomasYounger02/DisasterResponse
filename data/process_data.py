...
def clean_data(df):
    ...
    row = categories[0:1]
    category_colnames = [name.split('-')[0] for name in row.values.tolist()[0]]
    ...
    for column in categories:
        ...
        # deal with the 2
        categories[column] = np.where(categories[column]<=1, categories[column], 1)
...
def save_data(df, database_filename):
    ...
    cur = conn.cursor()
    cur.execute("DROP TABLE IF EXISTS DisasterMessage")
    # save data
